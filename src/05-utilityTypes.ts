...
import {Friend, Colleague, SecureFriendContact, FriendPartial, EventPass, CombinedFriendColleague} from "./myTypes";
...
): CombinedFriendColleague[] {
    let result: CombinedFriendColleague[] = []
    friends.reduce((res, friend) => {
        const colleague = colleagues.find((col) => col.name === friend.name);
        if (colleague) {
            const combined: CombinedFriendColleague = {
                name: friend.name,
                age: friend.age,
